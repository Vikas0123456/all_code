...
var dbModel = {
    ...
    delete: async (data_id,submerchant_id) => {
        ...
        let qb = await pool.get_connection();
        response = await qb
        .query("delete from "+dbtable+" where id="+data_id);
        qb.release();
        return response;
    }, 
    ...
    get_count: async (condition_obj,data_id) => {
        ...
        let qb = await pool.get_connection();
        let response = await qb
        .query("select count('id') as count from "+dbtable+" where id !="+data_id+" and  "+ condition);
        qb.release();
      console.log()
    ...
    get_dynamic_count: async (condition_obj,data_id,table_name) => {
        ...
        let qb = await pool.get_connection();
        let response = await qb
        .query("select count('id') as count from "+table_name+" where id !="+data_id+" and  "+ condition);
        qb.release();
    ...
    add_dynamic: async (data,table_name) => {
        table_name = config.table_prefix+table_name;
        let qb = await pool.get_connection();
        let response = await qb.insert(table_name,data);
        qb.release();
        return response;
    },
    delete_all:async(table_name)=>{
        table_name = config.table_prefix+table_name;
        let qb = await pool.get_connection();
        let response = await qb.delete(table_name);
        qb.release();
    ...
    get_count_check: async (condition_obj) => {
        ...
        let condition = await helpers.get_conditional_string(condition_obj);
        let qb = await pool.get_connection();
        let response = await qb.select('id').where(condition).get(dbtable)
        qb.release();
