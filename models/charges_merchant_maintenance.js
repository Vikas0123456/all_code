...
getfeaturesName:async(mcc_codes)=>{
    ...
    for(i of mcc_codes_array){
      ...
      let qb = await pool.get_connection();
      response = await qb.query("select GROUP_CONCAT(feature) as name from "+mcc_dbtable+" where id in ("+new_mcc_codes_array.join(',')+")");
      qb.release();
  ...
  getcurrencyName1:async(mcc_codes)=>{
    ...
    for(i of mcc_codes_array){
      ...
      let qb = await pool.get_connection();
      response = await qb.query("select GROUP_CONCAT(code) as name from "+currency_dbtable+" where id in ("+new_mcc_codes_array.join(',')+")");
      qb.release();
      return response[0].name;
  },
  ...
  getcurrencyName: async (data) => {
    ...
    const response = await qb.select("id,code").from(currency_dbtable).get(data);
    qb.release();
    const result = {};
...
select: async (selection,limit,like_conditions) => {
    ...
    let qb = await pool.get_connection();
        ...
        var response = await qb.get(charges_table);
...
select_features: async (selection,limit,like_conditions) => {
    ...
    let qb = await pool.get_connection();
        ...
        qb.limit(limit.perpage, limit.start)
        var response = await qb.get(features_table);
