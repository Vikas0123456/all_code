...
module.exports = async (dial,table_name,mobile) => {
    ...
    if (response[0].accept_zero_at_first_palce == 'Yes') {
        if (parseInt(f_letter) != 0) {
            return {status:false,message:"Please enter mobile number start with 0"};
        }
    ...
    if (response[0].mobile_no_length != mobile.length) {
        return {status:false,message:"Please enter "+response[0].mobile_no_length+" digit mobile number"};
    }
