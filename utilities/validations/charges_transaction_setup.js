...
const transaction = {
        ...
        if (req.body.charges_type == "Slab" || req.body.charges_type == "volume_Base") {
            if (checkEmpty(req.body, ["psp", "mcc", "currency", "payment_mode", "mcp_activation_fee", "mid_setup_fee", " mid_annual_fee", "monthly_tpv", "monthly_margin", "charges_type"])) {
                const schema = Joi.object().keys({
                    ...
                    fixed_amount_fraud: Joi.string().min(1).max(4).trim().required().error(() => {
                    ...
                    fixed_amount_processing: Joi.string().min(1).max(4).trim().required().error(() => {
    ...
    dynamic_order: async (req, res, next) => {
        if (checkEmpty(req.body, ["id"])) {

            const schema = Joi.object().keys({
                id: Joi.string().min(2).max(150).required().error(() => {
                    return new Error("Account ID required")
                }),
                super_merchant: Joi.string().min(2).max(150).optional().allow('').error(() => {
                    return new Error("")
                }),
                page:Joi.optional().allow('').error(() => {
                    return new Error("")
                }),
                perpage:Joi.optional().allow('').error(() => {
                    return new Error("")
                }),
            })
