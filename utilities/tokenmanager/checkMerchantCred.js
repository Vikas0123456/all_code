...
module.exports = async (req, res, next) => {
    ...
    if(!merchant_secret&&!merchant_key){
        ...
        let response = await qb
            .select('mk.merchant_id,mk.type,md.super_merchant_id,mcc.id as mcc_id,mcc_cat.id as mcc_cat_id')
            .from(config.table_prefix + 'master_merchant_key_and_secret mk')
            .join(config.table_prefix+"master_merchant md",'mk.merchant_id=md.id','inner')
            .join(config.table_prefix+"master_merchant_details mde","mk.merchant_id=mde.merchant_id","left")
            ...
            .where({'mk.merchant_key':merchant_key,'mk.merchant_secret':merchant_secret})
            .get();
        if(response[0]){
            ...
            res.status(StatusCode.badRequest).send(ServerResponse.validationResponse('Invalid merchant key and secret','E0001'));
        }
