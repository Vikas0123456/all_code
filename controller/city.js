...
var city = {
    add: async(req, res) => {
        ...
        CityModel.add(ins_body).then((result) => {
            let module_and_user = {
                user:req.user.id,
            ...
            admin_activity_logger.add(module_and_user,added_name,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('City added successfully.'));
            }).catch((error)=>{
    ...
    list: async(req, res) => {
        let limit
        if(req.bodyString('state_id') || req.bodyString('country_id')){
            limit = {
                perpage:0,
                page:0,
            }
        ...
        const state = await helpers.get_state_id_by_name(req.bodyString('state_name'));
        ...
        if (req.bodyString('country_name')){ search_obj.ref_country = country }

        if (req.bodyString('state_name')){ search_obj.ref_state = state }
        let search_city = {"city_name" :""}
        if (req.bodyString('city_name')){ search_city.city_name = req.bodyString('city_name') }
        CityModel.select(search_obj,search_city,limit)
            .then(async (result) => {
                ...
                let [all_country,all_state] = await Promise.all([helpers.get_country_name_by_ids(country_ids),
                    helpers.get_state_name_by_ids(state_ids)])
                ...
                total_count = await CityModel.get_count(search_obj,search_city)
              ...
              let send_res = [];
                let val = result
                    let res1 = {
                        ...
                        state_id: enc_dec.cjs_encrypt(val.ref_state),
                        state_name: await helpers.get_state_name_by_id(val.ref_state),
                        city_id: enc_dec.cjs_encrypt(val.id),
                        city_name:val.city_name,
                        status:(val.status==1)?"Deactivated":"Active",
                    };
    ...
    update: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.edit(module_and_user,city_id,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('City updated successfully'));
            }).catch((error)=>{
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
    ...
    deactivate: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.deactivate(module_and_user,city_id,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('City deactivated successfully'));
            }).catch((error)=>{
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
    ...
    activate: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.activate(module_and_user,city_id,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('City activated successfully'));
            }).catch((error)=>{
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
    ...
    delete: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.delete(module_and_user,city_id,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('City deleted successfully'));
            }).catch((error)=>{
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
