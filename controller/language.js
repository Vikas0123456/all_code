...
var language = {
    add: async(req, res) => { 
        ...
        LanguageModel.add(ins_body).then((result) => {
            let module_and_user = {
                user:req.user.id,
            ...
            .then(async (result) => {
                ...
                result.forEach(function(val,key) {
                    let res = {
                        ...
                        file:server_addr+':'+port+"/static/language/"+val.file,
                        flag:server_addr+':'+port+"/static/language/"+val.flag,
    ...
    update: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.edit(module_and_user,language_id,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('Language updated successfully'));
            }).catch((error)=>{
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch(error) {
    ...
    deactivate: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.deactivate(module_and_user,language_id,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('Language deactivated successfully'));
            }).catch((error)=>{
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
    ...
    activate: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.activate(module_and_user,language_id,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('Language activated successfully'));
            }).catch((error)=>{
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
    ...
    delete: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.delete(module_and_user,language_id,headers).then((result)=>{
                res.status(statusCode.ok).send(response.successmsg('Language deleted successfully'));
            }).catch((error)=>{
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
