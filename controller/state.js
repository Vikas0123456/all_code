...
var states = {
    add: async (req, res) => {
        ...
        if (result.length > 0) {
            ...
            StatesModel.add(ins_body).then((result) => {
                let module_and_user = {
                    user: req.user.id,
                ...
                admin_activity_logger.add(module_and_user, added_name, headers).then((result) => {
                    res.status(statusCode.ok).send(response.successmsg('State added successfully.'));
                }).catch((error) => {
    ...
    list: async (req, res) => {
        ...
        if (req.bodyString('state_name')) { search_state.state_name = req.bodyString('state_name') }
    ...
    update: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.edit(module_and_user, state_id, headers).then((result) => {
                res.status(statusCode.ok).send(response.successmsg('State updated successfully'));
            }).catch((error) => {
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
    ...
    states_deactivate: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.deactivate(module_and_user, state_id, headers).then((result) => {
                res.status(statusCode.ok).send(response.successmsg('State deactivated successfully'));
            }).catch((error) => {
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
    ...
    states_activate: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.activate(module_and_user, state_id, headers).then((result) => {
                res.status(statusCode.ok).send(response.successmsg('State activated successfully'));
            }).catch((error) => {
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
    ...
    states_delete: async (req, res) => {
        try {
            ...
            let headers = req.headers;
            admin_activity_logger.delete(module_and_user, state_id, headers).then((result) => {
                res.status(statusCode.ok).send(response.successmsg('State deleted successfully'));
            }).catch((error) => {
                res.status(statusCode.internalError).send(response.errormsg(error.message));
            })
        } catch {
